...
### Содержание
  - [Веб-интерфейс (GUI)](#веб-интерфейс-gui)
  - [Введение](#введение)
...
В проекте есть готовый графический веб-интерфейс для работы с Яндекс Музыкой — просмотр библиотеки, поиск и скачивание треков прямо из браузера.

#### Запуск в один клик

- **macOS** — дважды кликните по файлу `start.command`
- **Windows** — дважды кликните по файлу `start.bat`

Скрипт сам установит зависимости, запустит сервер и откроет браузер.

#### Возможности

- **Автоматический вход** — токен получается из cookies браузера, ничего копировать не нужно
- **Мне нравится** — просмотр лайкнутых треков с отметками о скачанных
- **Плейлисты** — просмотр и скачивание треков из плейлистов
- **Поиск** — по трекам, альбомам и артистам
- **Пакетное скачивание** — выбрать треки или скачать всё разом, с прогресс-баром
- **Настройки** — выбор папки для загрузок через визуальный файл-пикер
- **Сессия сохраняется** — повторный вход после перезагрузки не нужен

Подробнее в [документации GUI](gui/README.md).
...
Для работы с API необходим OAuth-токен. Есть несколько способов его получить:

**Автоматически (рекомендуется)** — если вы используете [веб-интерфейс](#веб-интерфейс-gui), токен получается автоматически из cookies браузера. Просто нажмите «Войти автоматически».

**Через Session_id cookie** — актуальный способ на 2025+ год:

1. Откройте [music.yandex.ru](https://music.yandex.ru/) и войдите в аккаунт
2. Откройте DevTools (`F12` или `Cmd+Option+I`)
3. Вкладка **Application** → **Cookies** → `.yandex.ru` → скопируйте значение `Session_id`
...
print(resp.json()['access_token'])  # токен действителен 1 год
```

**Через браузерное расширение** — [yandex-music-token](https://github.com/MarshalX/yandex-music-token): расширения для Chrome и Firefox, APK для Android.

Подробнее о получении токена — в [документации](docs/source/token.md).
